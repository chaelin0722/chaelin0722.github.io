...
[위키독스](https://wikidocs.net/64066)라는 곳에서 자연어 처리를 위한 딥러닝 개념들을 잘 기술한것 같아 이 사이트를 참고해 공부하였다. 
...
Convolutional layer는 local receptive fields와 shared weight의 조합으로 Convolution 계산을 하는 층이다. 이 층은 이전 층을 토대로 feature map을 생성하게 되는데, 
...
Subsampling layer는 Pooling Layer과 같은 개념이다. Convolutional layer와 달리 이전 층의 한 feature map에게서만 영향을 받는다. 
