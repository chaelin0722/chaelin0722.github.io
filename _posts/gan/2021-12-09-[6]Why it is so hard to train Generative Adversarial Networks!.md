...
$f = x \mul y$ 가 있다고 할때, 한 플레이어는 x에 대한 통제력이 있고 목표값 f를 최대화하려고 한다. 다른 플레이어는 y에 대한 통제력이 있고 목표값 f 를 최소화 하려고 한다. 이 함수를 시각화 하면 아래와 같다. ( [함수 시각화된 사이트 여기!](https://www.math3d.org/wz85eIlP) )

![image](https://user-images.githubusercontent.com/53431568/147206377-22039697-2dcb-4e36-ac49-50c29a142e7c.png)


$f = x \mul y$ 의 표면이 마치 말에 얹어진 안장모양이다. 한쪽 방향으로는 값이 커지다가 다시 떨어지고 다른 방향에서는 값이 떨어지다가 다시 올라가는 그런 모양이다. 
