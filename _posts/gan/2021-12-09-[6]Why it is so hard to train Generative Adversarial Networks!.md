...
이 두 값을 계속 갱신 규칙에 적용하게 되면 수렴하지 않고 진폭이 커지며 진동하는 현상을 볼 수 있다. 다른 초깃값을 넣어도 같은 현상이 나타난다. 이는 상당히 좋지 않은 결과이다. 경사 하강법이 간단한 min-max 게임에 대해 좋은 해결을 찾지 못할 뿐더러 수렴의 반대인 발산으로 나타나기 때문입니다.
...
이처럼 경사 하강법이 적대 게임에서 아주 간단한 목적 함수에 대해서도 최적의 해결책을 찾지 못하고 실패하는 경우를 살펴보았는데요, 경사 하강법은 일반적인 환경에서 global minima는 아니더라도 local minima 같은 값을 하나는 찾도록 보장이 되어있는 함수이다. 

유의미가 있는 데이터가 있는 현실적인 GAN은 손실 표면이 훨신 복잡할 것이고 이것이 발산으로 빠질 가능성을 줄여줄 수 는 있다. 하지만 이 과정에서 gan을 훈련시키는 것이 왜 어려운지, 훈련이 왜 혼란으로 바뀔 수 있는지 또한 알았다. 최적의 해결책 주위를 원을 그리며 도는 현상은 단순한 GAN이 더 나은 퀄리티의 이미지를 만들어내는 대신, 모드 붕괴에 빠져버리는 것으로 귀결되는지에 대한 설명이 될 수 있다. 잘 작동하는 경우가 있긴 하지만 근본적으로 경사하강볍은 GAN에는 맞지 않는 방법이다. GAN에 딱 맞는 최적화 방법에 대해서는 현재도 활발하게 논의중에 있다. 
